...
public class MobileServiceTester {
	...
	public static void registerRequest() {
		...
		try {
			ServiceRequest obj = new ServiceRequest("Abc", Arrays.asList("Battery"), 9876543210L, "Chap",
					3214567890123456L);
			obj = service.registerRequest(obj);
			System.out.println("Service request has been accepted with the request ID: " + obj.getServiceId()
					+ ", status of the request is " + obj.getStatus() + " on "
					+ obj.getTimeOfRequest().format(DateTimeFormatter.ofPattern("dd/MMM/yyyy  hh:mm:ss")));
